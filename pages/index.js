...
export default function Home() {
  ...
  const login = () => {
    const authUrl = `${SPOTIFY_CONFIG.AUTH_ENDPOINT}?client_id=${SPOTIFY_CONFIG.CLIENT_ID}&redirect_uri=${SPOTIFY_CONFIG.REDIRECT_URI}&response_type=${SPOTIFY_CONFIG.RESPONSE_TYPE}&scope=${encodeURIComponent(SPOTIFY_CONFIG.SCOPES)}&show_dialog=true`;
  ...
  const searchTracks = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://api.spotify.com/v1/search?q=${encodeURIComponent(searchKey)}&type=track&limit=10`,
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8">
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-lg p-6">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-center mb-4">Spotify Player</h1>
        </div>
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                {!isPlayerReady && (
                  <p className="text-sm text-yellow-600">正在初始化播放器...</p>
                )}
                {initError && (
