...
const newsItems = [
  {
    ...
    description: '從台中場的滿場驚呼聲當中開始，在歷經了兩個月的時間後，音樂劇《勸世三姊妹》原聲帶集資發行計畫在11月29日 23:59正式告一段落！真的非常感謝大家對集資計畫的支持與關愛，製作團隊們全都感受到了，我們帶著大家滿滿的心意與期待，持續馬不......',
    image: '/01.jpg'
  },
  {
      date: '2024/02/01',
  ...
  return (
    <div className={styles.container}>
      <ul className={styles.list}>
        <li 
          className={`${styles.item} ${activeTab === 'content' ? styles.active : ''}`}
          onClick={() => setActiveTab('content')}
        >
          <h6>專案內容</h6>
        </li>
        <li 
          className={`${styles.item} ${activeTab === 'faq' ? styles.active : ''}`}
          onClick={() => setActiveTab('faq')}
        >
          <h6>常見問題</h6>
        </li>
        <li 
          className={`${styles.item} ${activeTab === 'news' ? styles.active : ''}`}
